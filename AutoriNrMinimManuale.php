...
$sql = "SELECT A.Nume as Nume, A.Prenume as Prenume, count(AM.AutorID) as NrManuale
FROM Autor A join autormanual AM on A.AutorID = AM.AutorID
Group by A.Nume, A.Prenume
Having count(AM.ManualID) >= 2";
echo "<center><h1>Autorii care au publicat cel putin doua manuale</h1></center>";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0)
	{	while($row = mysqli_fetch_assoc($result))
		{
		echo "<div><tr><td>" . $row["Nume"] . "</td>   <td>" . $row["Prenume"] . "</td>   <td>" . $row["NrManuale"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
