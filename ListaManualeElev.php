...
//$CNP = $_POST['CNP'];
$sql = "SELECT M.Denumire as NumeManual, Ed.Denumire as NumeEditura
FROM Manual M join Editura Ed on M.Editura_ID = Ed.EdituraID
			  join Exemplar Ex on M.ManualID = Ex.ManualID
              join Elev Ev on Ex.ElevID = Ev.ElevID
WHERE Ev.Nume = '$Nume' AND Ev.Prenume = '$Prenume'";

echo "<center><h1>Lista manualelor folosite de $Nume $Prenume</h1></center>";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0)
	{	while($row = mysqli_fetch_assoc($result))
		{
		echo "<div><tr><td>" . $row["NumeManual"] . "</td>   <td>" . $row["NumeEditura"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
