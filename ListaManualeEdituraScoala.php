...
<div class="w3-container" style="margin-left:250px">
  <table class="w3-table-all">
    <tr>
      <th>Numar de manuale</th>
    </tr>
<?php
include_once 'connect.php';
$Scoala = $_POST['Scoala'];
$Editura = $_POST['Editura'];
$sql = "SELECT Count(Ev.ScoalaID) as NrManuale
FROM Elev Ev join Scoala S on Ev.ScoalaID = S.ScoalaID
			 join Exemplar Ex on Ev.ElevID = Ex.ElevID
             join Manual M on Ex.ManualID = M.ManualID
             join Editura Ed on Ed.EdituraID = M.Editura_ID
WHERE S.Denumire = '$Scoala' AND Ed.Denumire = '$Editura'";
$result = mysqli_query($conn, $sql);
echo "<center><h1>Numarul de manuale de la editura $Editura care se folosesc la $Scoala</h1></center>";
if (mysqli_num_rows($result) > 0)
	{	while($row = mysqli_fetch_assoc($result)) 
		{
		echo "<div><tr><td>" . $row["NrManuale"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
...
<div class="content">
		<!--<img class="img-responsive" src="studentlogo.png" height = "800" width = "800" alt="studentlogo">-->
		</div>
	</body>
</html>
