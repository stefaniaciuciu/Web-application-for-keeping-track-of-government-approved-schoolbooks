...
echo "<center><h1>Informatii despre manualele acceptate de minister</h1></center>";
$sql = "SELECT M.Denumire as Denumire, M.Materie as Materie, M.Clasa as Clasa, Ed.Denumire as Editura
		FROM Manual M join Editura Ed on M.Editura_ID = Ed.EdituraID
		GROUP BY M.Denumire
		";
...
if (mysqli_num_rows($result) > 0)
	{	 
		...
		{
		echo "<div><tr><td>" . $row["Denumire"] . "</td>   <td>" . $row["Materie"] . "</td>   <td>" . $row["Clasa"] . "</td>   <td>" . $row["Editura"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
  ...
  <br><br>
   <table class="w3-table-all">
    <tr>
      <th>Manual</th>
	   <th>Editura</th>
	    <th>Numar de elevi</th>

    </tr>
<?php
include_once 'connect.php';
echo "<center><h1>Cel mai folosit manual de catre elevi</h1></center>";
$sql2 = "SELECT M.Denumire as Manual, Ed.Denumire as Editura, count(Ex.ManualID) as NrElevi
FROM Manual M join Editura Ed on M.Editura_ID = Ed.EdituraID
			  join Exemplar Ex on M.ManualID = Ex.ManualID
GROUP BY Ex.ManualID
HAVING count(Ex.ManualID) >= ALL(SELECT count(Exx.ManualID)
                              FROM Exemplar Exx
                              GROUP BY Exx.ManualID)";
...
if (mysqli_num_rows($result2) > 0)
	{	 
		...
		{
		echo "<div><tr><td>" . $row["Manual"] . "</td>   <td>" . $row["Editura"] . "</td>   <td>" . $row["NrElevi"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
