...
echo "<center><h1>Numele Elevilor care au toate manualele de la $Editura</h1></center>";
$sql = "SELECT E.Nume as Nume, E.Prenume as Prenume, E.Clasa as Clasa
FROM Elev E join exemplar Ex on E.ElevID = Ex.ElevID
			join Manual M on Ex.ManualID = M.ManualID
            join Editura Ed on M.Editura_ID = Ed.EdituraID
WHERE M.Denumire in (SELECT MM.Denumire
                     FROM Manual MM join Editura Edd on MM.Editura_ID = Edd.EdituraID
                     WHERE Edd.Denumire = '$Editura')";
...
if (mysqli_num_rows($result) > 0)
	{	
		...
		{
		echo "<div><tr><td>" . $row["Nume"] . "</td>   <td>" . $row["Prenume"] . "</td>   <td>" . $row["Clasa"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
