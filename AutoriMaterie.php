...
<?php
$conn = new mysqli("localhost:3307","root","","evidentamanuale") or die("Unable to connect");
$Materie = $_POST["Materie"];
//$sql = "SELECT A.Nume, A.Prenume FROM autor A join AutorManual AM on A.AutorID = AM.AutorID join Manual M on M.ManualID = AM.ManualID WHERE M.Materie = '$Materie';";
$sql = "SELECT A.Nume, A.Prenume FROM autor A join autormanual AM on A.AutorID = AM.AutorID join manual M on M.ManualID = AM.ManualID WHERE M.Materie = '$Materie';";
echo "<center><h1>Autorii care au publicat manuale $Materie</h1></center>";
...
if (mysqli_num_rows($result) > 0)
	{	//$row = mysqli_fetch_assoc($result);
		...
		{
		echo "<div><tr><td>" . $row["Nume"] . "</td>   <td>" . $row["Prenume"] . "</td></tr></div>";
		}
	}
	else {
		echo "No results";
